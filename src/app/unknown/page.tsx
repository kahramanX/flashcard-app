...
export default async function UnknownLevelsIndexPage() {
  await new Promise(resolve => setTimeout(resolve, 500));
  const levelCounts = await api.getUnknownLevelCounts();
    ...
    <main className="min-h-screen p-4 sm:p-8 bg-gray-100 dark:bg-zinc-900 text-gray-900 dark:text-white flex flex-col">
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-300 dark:border-zinc-700">
          <h1 className="text-3xl font-bold">Unknown Words by Level</h1>
          <Link 
            href="/"
        ...
        {sortedLevels.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {sortedLevels.map(level => (
              <Link
                key={level}
                href={`/unknown/${encodeURIComponent(level)}`}
                prefetch={false}
